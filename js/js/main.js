...
function processCompleteMessage(data) {
    ...
    switch (data.type) {
        case 'telemetry':
            // Merge incoming telemetry with last saved telemetry so sparse updates don't drop quaternion/other fields
            let mergedTelemetry = Object.assign({}, (window.telemetryData || {}), data || {});
            // Normalize any shorthand telemetry fields (ta -> trim_angle, rt -> roll_trim, sp -> speed, etc.)
            mergedTelemetry = normalizeTelemetryData(mergedTelemetry);
            // If we now have a quaternion (either from the incoming message or preserved), compute Eulers
            if (typeof mergedTelemetry.qw === 'number' && typeof mergedTelemetry.qx === 'number' && typeof mergedTelemetry.qy === 'number' && typeof mergedTelemetry.qz === 'number') {
                const eul = computeEulerFromQuaternion(mergedTelemetry.qw, mergedTelemetry.qx, mergedTelemetry.qy, mergedTelemetry.qz);
                if (eul) {
                    // Keep raw Euler values for logic and charts
                    mergedTelemetry.raw_pitch = eul.pitch;
                    mergedTelemetry.raw_yaw = eul.yaw;
                    mergedTelemetry.raw_roll = eul.roll;
                    // Now apply model mapping to generate visualization Euler (does not affect control logic)
                    const mapped = applyModelMappingToEuler(eul);
                    mergedTelemetry.viz_pitch = mapped.pitch;
                    mergedTelemetry.viz_yaw = mapped.yaw;
                    mergedTelemetry.viz_roll = mapped.roll;
                    // Compute displayed pitch/roll after reading any available trim values (may be preserved in merged telemetry)
                    const currentTrim = (mergedTelemetry.trim_angle !== undefined) ? Number(mergedTelemetry.trim_angle) : Number((window.telemetryData && window.telemetryData.trim_angle) || parseFloat(document.getElementById('trimValueDisplay')?.textContent || '0') || 0);
                    const currentRollTrim = (mergedTelemetry.roll_trim !== undefined) ? Number(mergedTelemetry.roll_trim) : Number((window.telemetryData && window.telemetryData.roll_trim) || parseFloat(document.getElementById('rollTrimValueDisplay')?.textContent || '0') || 0);
                    mergedTelemetry.pitch = (mergedTelemetry.raw_pitch || 0) + (isNaN(currentTrim) ? 0 : currentTrim);
                    mergedTelemetry.yaw = mergedTelemetry.raw_yaw;
                    mergedTelemetry.roll = (mergedTelemetry.raw_roll || 0) + (isNaN(currentRollTrim) ? 0 : currentRollTrim);
                }
            }
            // Persist merged telemetry, then update UI + charts with merged data
            updateTelemetryUI(mergedTelemetry);
            updateChart(mergedTelemetry);
            updateActualPath(mergedTelemetry);
...
function updateTelemetryUI(data) {
    ...
    // Merge with previous telemetry so sparse updates don't overwrite quaternion/raw values
    window.telemetryData = Object.assign({}, (window.telemetryData || {}), data || {});
    data = window.telemetryData; // work with merged telemetry for this update
