...
let joystickKeepAliveInterval = null;
...
function handleJoystickStart(event) { event.preventDefault(); isDragging = true; if (joystickKeepAliveInterval) clearInterval(joystickKeepAliveInterval); joystickKeepAliveInterval = setInterval(() => { if (isDragging) { const now = Date.now(); if (now - lastJoystickSendTime > JOYSTICK_SEND_INTERVAL) { sendBleMessage({ type: 'joystick', x: currentJoystickX, y: currentJoystickY }); lastJoystickSendTime = now; } } }, 100); }
...
function handleJoystickEnd(event) { if (!isDragging) return; event.preventDefault(); isDragging = false; if (joystickKeepAliveInterval) { clearInterval(joystickKeepAliveInterval); joystickKeepAliveInterval = null; } drawJoystick(document.getElementById('joystickCanvas').getContext('2d'), joystickCenter.x, joystickCenter.y); sendBleMessage({ type: 'joystick', x: 0, y: 0 }); }
