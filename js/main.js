 ...
 * - Logs if message not sent due to no connection
 */
function sendBleMessage(message) {
    try {
        const appIsConnected = (typeof appStore !== 'undefined' && appStore.getState && appStore.getState('connection.isConnected')) || false;
        const layerIsConnected = (typeof commLayer !== 'undefined' && commLayer && typeof commLayer.getConnectionStatus === 'function' && commLayer.getConnectionStatus());
        if (appIsConnected || layerIsConnected) {
            try { commLayer.send(message); } catch (err) { console.warn('[sendBleMessage] commLayer.send failed', err, message); }
        } else {
            try { addLogMessage(`[UI] Nie wysłano wiadomości (brak połączenia): ${message.type || JSON.stringify(message)}`, 'warn'); } catch (e) { console.warn('[sendBleMessage] no log available'); }
        }
    } catch (err) {
        console.error('[sendBleMessage] error', err);
    }
}
...
function setupCommunicationHandlers() {
    ...
    // If lower layer notifies about connection, reflect it into AppState
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startGaTuning) {
        startGaTuning.addEventListener('click', async () => {
            ...
            try {
                // Use tuning algorithms directly (they are loaded globally)
    ...
    if (startPsoTuning) {
        startPsoTuning.addEventListener('click', async () => {
            ...
            try {
                // Use tuning algorithms directly
