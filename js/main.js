...
// Communication classes moved to 'js/communication_layer.js' (canonical).
// Keep runtime aliases for backwards compatibility in case other files depend on global names.
(function () {
    if (typeof BLECommunication === 'undefined' && typeof window !== 'undefined' && window.BLECommunication) {
        // Use a non-const var so we don't shadow the global if already present
        var BLECommunication = window.BLECommunication;
    }
    if (typeof MockCommunication === 'undefined' && typeof window !== 'undefined' && window.MockCommunication) {
        var MockCommunication = window.MockCommunication;
    }
})();
...
function handleJoystickStart(event) {
    ...
    isDragging = true;
    // capture current pointer position and send one immediate message, then start continuous loop
    const joystickCanvas = document.getElementById('joystickCanvas');
